#Get Playable Date (GET)
#Includes both positive and negative test cases.
#Created by Tarja Rechsteiner on 1.11.10.
...
class GetPlayableDate(TestSuiteBase):
    ...
    def test_validInfo(self):
        ...
        result = self.toolBox.getPlayableDate(billingId)
        
        self.assertTrue(result.httpStatus() == 200,\
                        "http status code: " + str(result.httpStatus())) 
        self.assertTrue("date" in result, "XML failed: date not found in result: " + str(result))
        self.assertTrue("value" in result['date'], "XML failed, value not found in result: " + str(result))
        
        self.assertTrue(result['date']['value'][:2] == "20", "Year returned does not start with 20: " + str(result))
        self.assertTrue(result['date']['value'][2:4] >= '10', "Year returned is not greater than 2009: " + str(result))
        self.assertTrue(result['date']['value'][5:7] >= '01', "Month returned less than 1: " + str(result))
        self.assertTrue(result['date']['value'][5:7] <= '12', "Month returned greater than 12: " + str(result))
        self.assertTrue(result['date']['value'][8:10] >= '01', "Day returned less than 1: " + str(result))
        self.assertTrue(result['date']['value'][8:10] <= '31', "Month returned greater than 31: " + str(result))
        self.toolBox.scriptOutput("getPlayableDate valid account", {"billingId": billingId, "Date returned": result['date']['value']})
    ...
    def test_validPaypalInfo(self):
        ...
        self.assertTrue(result['date']['value'][8:10] >= '01', "Day returned less than 1: " + str(result))
        self.assertTrue(result['date']['value'][8:10] <= '31', "Month returned greater than 31: " + str(result))
        self.toolBox.scriptOutput("getPlayableDate valid paypal account", {"billingId": billingId, "Date returned": result['date']['value']})
    ...
    def test_invalidInfo(self):
        '''Invalid information -- TC5'''
        result = self.toolBox.getPlayableDate('0000000000000000')
    ...
    def test_missingParams(self):
        ...
        result = self.toolBox.blankGet('getPlayableDate')
    ...
    def test_emptyBillingId(self):
        '''Empty billingId -- TC10'''
        result = self.toolBox.getPlayableDate('')
    ...
    def infoFailCheck(self, result, billingId, titleCode='KFPW') :
        ...
        self.assertTrue(parameters['billingAcctId'] == billingId, "billingId returned not equal to billingId given: " + billingId + " " + str(parameters))
        self.assertTrue(parameters['service'] == "getPlayableDate", "Service returned not equal to service called: getPlayableDate " + str(parameters))
        if titleCode == None:
