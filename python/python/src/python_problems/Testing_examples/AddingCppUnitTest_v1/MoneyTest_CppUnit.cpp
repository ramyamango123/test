...
MoneyTest::testConstructor()
{
  ...
  CPPUNIT_ASSERT_EQUAL_MESSAGE("This test fails intentionally",
                                1+longNumber, money.getAmount() );
}
...
MoneyTest::testAdd()
{
  ...
  const Money wrongexpectedMoney( 136, "FF" );
  ...
  CPPUNIT_ASSERT( wrongexpectedMoney == money );           // add works
